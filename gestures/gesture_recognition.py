...
class GestureRecognition:
    ...
    def gesture_id_to_label(self, gesture_id):
        """
        Convert gesture ID → human-readable label from CSV
        """
        try:
            ...
            if 0 <= gid < len(self.keypoint_classifier_labels):
                return self.keypoint_classifier_labels[gid]

            if 0 <= gid < len(self.point_history_classifier_labels):
                return self.point_history_classifier_labels[gid]

        except Exception:
            pass

        return str(gesture_id)
